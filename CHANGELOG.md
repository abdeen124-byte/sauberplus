...
- Every static label and every dynamically-rendered string (toasts, confirm dialogs, empty states, validation/error messages, table/card text, badges) across all 8 admin pages now goes through the translation layer — including the Edge Function's fixed German error responses, mapped client-side to translation keys rather than making server code language-aware.
- Added a purely client-side "Scheduled"/"Expired" display status for announcements (derived from existing `start_date`/`end_date`, never written back to the database) so those two required status labels have somewhere real to appear without changing the stored `draft`/`active`/`hidden` enum or any CRUD/RLS logic.
- Cairo font (already used on the public site) added to every admin page for Arabic; RTL CSS overrides added for the handful of physical-position styles (sidebar slide direction, password-toggle button, toast stack, remember-me switch, login screen's decorative diagonal shape) that flexbox/grid's automatic `dir`-mirroring doesn't cover on its own.
- Verified via Playwright (desktop/tablet/mobile, both languages, mocked authenticated session): zero console errors on any of the 8 pages, correct `dir`/`lang` flip, correct RTL mirroring including the mobile sidebar's slide-from-right, and correct role-gated nav (Content Manager still can't see Team/Activity Log/Settings in either language).
