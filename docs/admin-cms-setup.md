...
- [ ] **The one step with no CLI/API path that doesn't require the `service_role` key: bootstrapping the first Super Admin.** `auth.users` row creation is only possible via the GoTrue Admin API (needs `service_role`) or the Dashboard's *Authentication → Users → Add user* — there is no CLI command for it, and the `admin-create-user` Edge Function can't bootstrap its own first caller (it requires an already-authenticated super admin to invoke it). This is confirmed as the one remaining manual action: Authentication → Users → Add user, email `Abdeen124@gmail.com`, then run the SQL insert into `user_profiles` below.
  ```sql
  insert into public.user_profiles (id, email, display_name, role)
  values ('PASTE-THE-UUID-HERE', 'Abdeen124@gmail.com', 'Super Admin', 'super_admin');
  ```
